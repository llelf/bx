...
ZS ma(I d,J n){ZJ p=0x700000;p+=d?0:n;R mmap(d?0:p-n,n,0x7,d?2:0x1002,d?d:-1,0);}ZS mf(S s,J*n){J b[18];I d=open(s,0);Qs(0>d,s)R fstat(d,b),s=(*n=b[9])?ma(d,*n):s,close(d),s;}
...
V km(S*a){N(26,G[i]=NL)*(K*)(K0=kK(0))=c0();if(*++a)pr(ld(*a));W(1)pr(es(r2(" ")));}V _start(){asm("movl $0x9fc0,-4(%rsp);ldmxcsr -4(%rsp);call _km");}
I main(I ac,S*av){_start(av);}
